	...
	$(document).ready(function(){
        ...
        jQuery('#ampaginationsm').on('am.pagination.change',function(e){
              jQuery('.showlabelsm').text('The selected page no: '+e.page);
                $(location).attr('href', "${pageContext.request.contextPath}/precedent_normal.do?rpage="+e.page);         
         });
		...
		<div class="reviewlist1">
			<table border=0 class="all">
				...
				<tr>
					<td>${vo.rno }</td>	
					<td></td>
					<td><a href="${pageContext.request.contextPath}/precedent_content.do?no=${vo.no }&rno=${vo.rno }">${vo.title }</a></td>
					<td>${vo.result }</td>
					<td>${vo.hits }</td>
				</tr>
				</c:forEach>
